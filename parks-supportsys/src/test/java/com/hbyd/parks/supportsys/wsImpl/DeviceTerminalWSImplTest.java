...
/**
 * 测试：DeviceTerminalWSImpl
 */
@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration({"/spring/supportsys/applicationContext_CXF_Client.xml"})
public class DeviceTerminalWSImplTest {

    private Gson gson = new Gson();

    @Resource
    private DeviceTerminalWS deviceTerminalWS;

    @Test
    public void testFindAll(){
        List<DeviceTerminalDTO> all = deviceTerminalWS.findAll();
...
//            System.out.println(gson.toJson(all.get(0)));
            for (DeviceTerminalDTO deviceTerminalDTO : all) {
                System.out.println(gson.toJson(deviceTerminalDTO));
            }
        }
    }
    ...
    public void testGetPageBean(){
        ...
        PageBeanEasyUI pageBean = deviceTerminalWS.getPageBean(queryBean, hql_where, params);
...
//      保存
        DeviceTerminalDTO dto = new DeviceTerminalDTO();
...
//        非空
        ...
        dto.setState("state");
//        特有
        dto.setInPortCount(1);

//      执行保存
        DeviceTerminalDTO saved = deviceTerminalWS.save(dto);
...
//      执行更新
        deviceTerminalWS.update(saved);

//      执行查找
        DeviceTerminalDTO updated = deviceTerminalWS.getByID(saved.getId());
//      断言
//        普通非空
        assert "one_type_new".equals(updated.getOneType());
//        普通特有
        assert 2 == updated.getInPortCount();
//        关联字段
...
//      执行删除
        deviceTerminalWS.delByID(saved.getId());
        assert deviceTerminalWS.getByID(saved.getId()) == null;
    }
