...
/**
 * 测试：DeviceReaderWSImpl
 */
@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration({"/spring/supportsys/applicationContext_CXF_Client.xml"})
public class DeviceReaderWSImplTest {

    private Gson gson = new Gson();

    @Resource
    private DeviceReaderWS deviceReaderWS;

    @Test
    public void testFindAll(){
        List<DeviceReaderDTO> all = deviceReaderWS.findAll();
...
//            System.out.println(gson.toJson(all.get(0)));
            for (DeviceReaderDTO deviceReaderDTO : all) {
                System.out.println(gson.toJson(deviceReaderDTO));
            }
        }
    }
    ...
    public void testGetPageBean(){
        ...
        PageBeanEasyUI pageBean = deviceReaderWS.getPageBean(queryBean, hql_where, params);
...
//      保存
        DeviceReaderDTO dto = new DeviceReaderDTO();
...
//      执行保存
        DeviceReaderDTO saved = deviceReaderWS.save(dto);
...
//        普通特有
        assert "rs".equals(saved.getReaderSn());
//        关联字段
...
//      执行更新
        deviceReaderWS.update(saved);

//      执行查找
        DeviceReaderDTO updated = deviceReaderWS.getByID(saved.getId());
//      断言
//        普通非空
        assert "one_type_new".equals(updated.getOneType());
//        普通特有
        assert "rs_new".equals(updated.getReaderSn());
//        关联字段
...
//      执行删除
        deviceReaderWS.delByID(saved.getId());
        assert deviceReaderWS.getByID(saved.getId()) == null;
    }
