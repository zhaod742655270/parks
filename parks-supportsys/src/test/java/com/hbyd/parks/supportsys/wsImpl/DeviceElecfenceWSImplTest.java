...
/**
 * 测试：DeviceElecfenceWSImpl
 */
@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration({"/spring/supportsys/applicationContext_CXF_Client.xml"})
public class DeviceElecfenceWSImplTest {

    private Gson gson = new Gson();

    @Resource
    private DeviceElecfenceWS deviceElecfenceWS;

    @Test
    public void testFindAll(){
        List<DeviceElecfenceDTO> all = deviceElecfenceWS.findAll();
...
//            System.out.println(gson.toJson(all.get(0)));
            for (DeviceElecfenceDTO deviceElecfenceDTO : all) {
                System.out.println(gson.toJson(deviceElecfenceDTO));
            }
        }
    }
    ...
    public void testGetPageBean(){
        ...
        PageBeanEasyUI pageBean = deviceElecfenceWS.getPageBean(queryBean, hql_where, params);
...
//      保存
        DeviceElecfenceDTO dto = new DeviceElecfenceDTO();
...
//      执行保存
        DeviceElecfenceDTO saved = deviceElecfenceWS.save(dto);
...
//        普通特有
        assert 12 == saved.getSn();
        assert "bi".equals(saved.getBeginInfo());
//        关联字段
...
//      执行更新
        deviceElecfenceWS.update(saved);

//      执行查找
        DeviceElecfenceDTO updated = deviceElecfenceWS.getByID(saved.getId());
//      断言
//        普通非空
        assert "one_type_new".equals(updated.getOneType());
//        普通特有
        assert "bi_new".equals(updated.getBeginInfo());
//        关联字段
...
//      执行删除
        deviceElecfenceWS.delByID(saved.getId());
        assert deviceElecfenceWS.getByID(saved.getId()) == null;
    }
