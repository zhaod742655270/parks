...
import com.hbyd.parks.dto.supportsys.DeviceIoDTO;
import com.hbyd.parks.ws.supportsys.DeviceIoWS;
...
public class DeviceIoWSImplTest {

    private Gson gson = new Gson();

    @Resource
    private DeviceIoWS deviceIoWS;

    @Test
    public void testFindAll(){
        List<DeviceIoDTO> all = deviceIoWS.findAll();
...
//            System.out.println(gson.toJson(all.get(0)));
            for (DeviceIoDTO deviceIoDTO : all) {
                System.out.println(gson.toJson(deviceIoDTO));
            }
        }
    }
    ...
    public void testGetPageBean(){
        ...
        PageBeanEasyUI pageBean = deviceIoWS.getPageBean(queryBean, hql_where, params);
...
//      保存
        DeviceIoDTO dto = new DeviceIoDTO();
...
//      执行保存
        DeviceIoDTO saved = deviceIoWS.save(dto);
...
//        普通特有
        assert "p".equals(saved.getPort());
        assert "st".equals(saved.getSignalType());
        assert "io".equals(saved.getInOut());
        assert "status".equals(saved.getStatus());
        assert "ti".equals(saved.getTimeId());
        assert "idown".equals(saved.getIsDown());
//        关联字段
...
//        普通特有
        saved.setPort("p_new");
        saved.setSignalType("st_new");
        saved.setInOut("io_new");
        saved.setStatus("status_new");
        saved.setTimeId("ti_new");
        saved.setIsDown("idown_new");
//        关联字段
...
//      执行更新
        deviceIoWS.update(saved);

//      执行查找
        DeviceIoDTO updated = deviceIoWS.getByID(saved.getId());
//      断言
//        普通非空
        assert "one_type_new".equals(updated.getOneType());
//        普通特有
        assert "p_new".equals(updated.getPort());
        assert "st_new".equals(updated.getSignalType());
        assert "io_new".equals(updated.getInOut());
        assert "status_new".equals(updated.getStatus());
        assert "ti_new".equals(updated.getTimeId());
        assert "idown_new".equals(updated.getIsDown());
//        关联字段
...
//      执行删除
        deviceIoWS.delByID(saved.getId());
        assert deviceIoWS.getByID(saved.getId()) == null;
    }
