...
$(function(){
    ...
    $('#company-dg').datagrid({
        ...
        singleSelect:true,
        pagination: true,
        url:'warehouseCompanyIn/warehouseCompanyList',
        frozenColumns:[[
 ...
 */
function addCompany(){
    formUrl = "warehouseCompanyIn/addWarehouseCompany";
    $('#addCompany').form('clear');
    $('#company-dlg').dialog('open').dialog('setTitle', '新增供应商信息');
}
...
function editCompany(){
    formUrl = "warehouseCompanyIn/editWarehouseCompany";
    ...
    if(row){
        ...
        $('#company-dlg').dialog('open').dialog('setTitle', '修改供应商信息');
    }else{
        $.messager.alert('提示', '需要选择一条供应商信息，才能进行编辑操作。', 'info');
    }
}
...
function deleteCompany(){
    ...
    if(row){
        $.messager.confirm('确认', '是否删除该条数据？',function(r){
            if(r){
                $.post("warehouseCompanyIn/deleteWarehouseCompany",{
                    id:row.id
                },function(data){
        ...
        $.messager.alert('提示', '需要选择一条供应商信息，才能进行删除操作。', 'info');
    }
}